...
class Revenue extends MainModel
{
        ...
        if(1<=$month&&$month<=12) {
            ...
            switch ($month) {
                case 1:
                    ...
                    $sql = "SELECT SUM(order_product.total_price) FROM `order_product` WHERE order_product.updated BETWEEN '$startInterval' AND '$endInterval' AND order_product.status=5";
                    echo $sql;
    ...
    /**Tính tổng số lượng đã bán của từng mặt hàng trong các tháng trong năm
     * @param $month
     * @return array
     */
